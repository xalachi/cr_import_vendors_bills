...
class AccountInvoice(models.Model):
    ...
    from_mail = fields.Boolean(
        "Desde email", help="If flagged, some fields of the invoice " "they will be read only "
    )

    has_ack = fields.Boolean(
        "Tiene ACK?", help="If flagged, indicates you have the " "answer (ACK) attached "
    )
    ...
    activity_id = fields.Many2one(context={"active_test": False})
